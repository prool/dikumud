...
void clone_char(struct char_data *ch)
{
	...
	GET_NAME(clone) = strdup_(GET_NAME(ch));

	clone->player.short_descr =	strdup_(ch->player.short_descr);

	clone->player.long_descr = strdup_(ch->player.long_descr);

	clone->player.description = 0;
	/* REMEMBER EXTRA DESCRIPTIONS */

	GET_TITLE(clone) = strdup_(GET_TITLE(ch));
...
void clone_obj(struct obj_data *obj)
{
	...
	*clone = *obj;
	...
	clone->name               = strdup_(obj->name);
	clone->description        = strdup_(obj->description);
	clone->short_description  = strdup_(obj->short_description);
	clone->action_description = strdup_(obj->action_description);
